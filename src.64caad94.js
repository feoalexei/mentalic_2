...
},{"@popperjs/core":"vePw"}],"RqnE":[function(require,module,exports) {
"use strict";
...
class ScrollBarHelper {
  ...
  _resetElementAttributes(selector, styleProp) {
    const manipulationCallBack = element => {
      const value = _manipulator.default.getDataAttribute(element, styleProp);
...
class Backdrop {
  ...
  _append() {
    ...
    _eventHandler.default.on(this._getElement(), EVENT_MOUSEDOWN, () => {
      (0, _index.execute)(this._config.clickCallback);
    });
...
class FocusTrap {
  ...
  activate() {
    ...
    _eventHandler.default.on(document, EVENT_FOCUSIN, event => this._handleFocusin(event));

    _eventHandler.default.on(document, EVENT_KEYDOWN_TAB, event => this._handleKeydown(event));
  ...
  _handleFocusin(event) {
    ...
    const elements = _selectorEngine.default.focusableChildren(trapElement);
...
var Lightbox = /*#__PURE__*/function () {
  ...
  _createClass(Lightbox, [{
    ...
    value: function getGalleryItems() {
      ...
      var gallery = galleryTarget ? _toConsumableArray(new Set(Array.from(document.querySelectorAll(`[data-gallery="${galleryTarget}"]`), function (v) {
        return `${v.dataset.type ? 'embed' : ''}${_this3.getSrc(v)}`;
      }))) : [this.src];
      return gallery;
    }
  }, {
    ...
    value: function createCarousel() {
      ...
      var slidesHtml = this.sources.map(function (src, i) {
        ...
        var inner = `<img src="${src}" class="d-block w-100 h-100 img-fluid" style="z-index: 1; object-fit: contain;" onload="${onload}" />`;
        var attributes = '';
    ...
    key: "randomHash",
    value: function randomHash(length = 8) {
      return Array.from({
        length: length
      }, function () {
...
var burgerToggle = document.querySelector('.navbar-toggler');
var headerTitle = document.querySelector('.header-title');
var headerContent = document.querySelector('.header-content');
var headerMedia = document.querySelector('.header-media');
burgerToggle.addEventListener("click", function () {
  if (window.screen.width <= 576) headerContent.classList.toggle('d-none');
  headerTitle.classList.toggle('d-none');
  headerMedia.classList.toggle('d-none');
});
/* ---- Toggle hamburger open/close button ---- */

var burgerLines = document.querySelector('.navbar-toggler-icon');
var burgerClose = document.querySelector('.bi-x-lg');
burgerToggle.addEventListener("click", function () {
...
var modalTitle = document.querySelectorAll('.modal'); // let alert = document.querySelectorAll('.alert');
...
Array.prototype.slice.call(forms).forEach(function (form) {
  form.addEventListener('submit', function (event) {
    if (!form.checkValidity()) {
      ...
      // event.preventDefault()
      // event.stopPropagation()
      swal("Спасибо!", "Ваша заявка была успешно отправлена. Мы свяжемся с Вами в течение 24 часов", "success"); // alert("Ваша заявка была успешно отправлена. Мы свяжемся с Вами в течение 24 часов");
    }

    form.classList.add('was-validated');
  }, false);
});
},{"bootstrap":"xqrD","bs5-lightbox":"Ljie"}]},{},["Focm"], null)
//# sourceMappingURL=src.64caad94.js.map